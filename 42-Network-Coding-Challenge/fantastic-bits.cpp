...
constexpr int OBLIVIATE = 0; // 対象のBludgerを3ターンの間，自分の追撃を停止させる．
constexpr int PETRIFICUS = 1; // 対象の動きを止める．
constexpr int ACCIO = 2; // Snaffle を Wizard に引き寄せる．
constexpr int FLIPENDO = 3; // Snaffle を押し込む．
constexpr int WINGARDIUM = 4; // 対象を目的の場所に押し込む．
...
Wizard* myWizard1;
Wizard* myWizard2;
Wizard* opponentWizard1;
Wizard* opponentWizard2;
Point* myGoal;
Point* opponentGoal;
Point* mid;
...
public:
    Wingardium(Wizard* caster, int type) {
        duration = 0;
        this->type = type;
        this->caster = caster;
    }
...
public:
	...
	// ユニットインスタンスと壁との次の衝突を検出する方法
	// シミュレーション時間の60％がここに費やされている．
...
public:
    ...
    Wizard(int team) {
        ...
        // spells[WINGARDIUM] = new Wingardium(this, WINGARDIUM);
	...
	// play 関数は1ゲームのターンをシミュレートする．
	// すべてのアクションを順番にプレイし，全員を動かせるようにします．
    void play();
...
void Wingardium::print() {
    if (duration)
        cerr << "Wingardium " << target->id << " " << duration << " | ";
}
...
// move 関数は、ボード上のユニットの変位や衝突を処理します．
void move() {
    double t = 0.0;
    double delta;
...
double eval() {
    // Hidden ;)
	// 実装すること
	// 01. 得点したゴールにはボーナスを加えることができるでしょう．1ターンで得たゴールは2ターンで得たゴールよりも優れていることを忘れないでください．
	// 02. ウィザードをスナッフルに近づけることは重要です．
	// 03. ウィザードを広げて広い範囲をカバーするのが良いでしょう．
	// 04. あなたのゾーンに戻って，あなたのゴールに近い2つの多くのスナッフルがあるときに防御を取得します．
	// 05. 相手のウィザードをスナッフルズから遠ざける．
	// 11. 自分と敵とのゴールの差
	// 12. スナッフルの距離の和
	// 13. 自身の魔法使いの最も近いスナッフルまでの距離の合計
	// 14. マナを予備に持っていることで、魔法を使うためのコスト感を与え、そうでなければAIが賢く使わなくなるため、より良いスコアを得ることができます。
	//     かろうじて動いているSnaffleにPetrificusをキャストするのは無駄だ。最初は保守的に考えすぎていたが、最終的にはマナをかなり安くして大規模な改善を見た。
	// 15. チームが勝つために必要なスナッフルである「重要な」スナッフルに、相手のゴールに最も近いスナッフルにもっと重みをかけてください。
	//     距離の計算を歪めて、ゴールやウィザードに近いと思わせるようにしました。
	//     最初はこれに重きを置きすぎて、AIを「オールイン」にしすぎてしまい、敵の計画を阻止する能力を過小評価してしまった。
	// 16. 私の2人のウィザードがお互いに近づきすぎたときの小さなペナルティ。
	//     これにより、2人のウィザードはより多くのスペースをカバーすることができ、特に同じスナッフルに手を伸ばすことを避けることができます。
	// 17. 距離を計算する際に速度を考慮に入れること。例えば、魔法使いとスナッフルは、互いに離れて移動している場合、同じ距離を持つべきではありません。
	// 18. 指数曲線はスナッフルのゴール距離に適用され、中距離が短距離とほぼ同じくらい脅威に見えるようにする。
	//     これにより、スナッフルが敵のゴールに向かってより素早く収束し、AIが自分のゴールへの脅威をより真剣に考えるようになる。
	// 19. 敵ウィザードのスナッフルまでの距離が大幅にマイナスになる。
	//     これは通常、敵ウィザードに突っ込むことでより防御的な行動につながり、敵に向かってスナッフルを投げることを回避する．
	double score = 0.0;
	// 以下は AI がスナッフルを自分のゴールに最も近づけることを可能にします．
	for (int i = 0; i < snafflesFE; ++i)
		if (!snaffles[i]->dead)
			score -= dist(snaffles[i], myGoal);
	if (!(myWizard1->snaffle)) {
		Snaffle* target = NULL;
		double targetD = INF;
		double d;
		for (int i = 0; i < snafflesFE; ++i) {
		...
		if (target)
			score -= dist(target, myWizard1);
	}
	if (!(myWizard2->snaffle)) {
		Snaffle* target = NULL;
		double targetD = INF;
		double d;
		for (int i = 0; i < snafflesFE; ++i) {
...
void reset() {
    ...
    myMagic = sMyMagic;
    myScore = sMyScore;
    opponentScore = sOpponentScore;
}

// すべての wizard が一番近い snaffle の方に行く．
//  wizard が snaffle を持っている場合、wizard は snaffle をゴールに向かって投げる．
void dummies() {
    if (opponentWizard1->snaffle)
        opponentWizard1->snaffle->thrust(500.0, opponentGoal, dist(opponentWizard1, opponentGoal));
    else {
        Snaffle* target = NULL;
        double targetD = INF;
        double d;
        for (int i = 0; i < snafflesFE; ++i) {
            Snaffle* snaffle = snaffles[i];
            if (!snaffle->dead) {
                d = dist2(opponentWizard1, snaffle);
                if (d < targetD) {
                    targetD = d;
                    target = snaffle;
                }
        ...
        if (target)
            opponentWizard1->thrust(150.0, target, sqrt(targetD));
    }

    if (opponentWizard2->snaffle)
        opponentWizard2->snaffle->thrust(500.0, opponentGoal, dist(opponentWizard2, opponentGoal));
    else {
        Snaffle* target = NULL;
        double targetD = INF;
        double d;
        for (int i = 0; i < snafflesFE; ++i) {
            Snaffle* snaffle = snaffles[i];
            if (!snaffle->dead) {
                d = dist2(opponentWizard2, snaffle);
                if (d < targetD) {
                    targetD = d;
                    target = snaffle;
                }
        ...
        if (target)
            opponentWizard2->thrust(150.0, target, sqrt(targetD));
    }
}
...
void simulate2(Solution* solution) {
    ...
    cerr << "myMagic: " << myMagic << endl;
    cerr << "myScore: " << myScore << endl;
    cerr << "opponentScore: " << opponentScore << endl;
...
int main() {
    ...
    if (!myTeamId) {
        myWizard1 = wizards[0];
        myWizard2 = wizards[1];
        opponentWizard1 = wizards[2];
        opponentWizard2 = wizards[3];
        myGoal = new Point(16000, 3750);
        opponentGoal = new Point(0, 3750);
    } else {
        myWizard1 = wizards[2];
        myWizard2 = wizards[3];
        opponentWizard1 = wizards[0];
        opponentWizard2 = wizards[1];
        myGoal = new Point(0, 3750);
        opponentGoal = new Point(16000, 3750);
    }
    ...
    while (1) {
		int myPoint;
		int myPower;
		cin >> myPoint >> myPower; cin.ignore();
		int oppoPoint;
		int oppoPower;
		cin >> oppoPoint >> oppoPower; cin.ignore();
        int entities;
        ...
        for (int i = 0; i < entities; i++) {
            ...
            if (entityType == "WIZARD" || entityType == "OPPONENT_WIZARD")
                unit = wizards[entityId];
            else if (entityType == "SNAFFLE") {
                if (!turn)
                    unit = new Snaffle();
                else
                    unit = unitsById[entityId];
                unit->dead = false;
                units[unitsFE++] = unit;
            ...
            unit->update(entityId, x, y, vx, vy, state);
        }
		...
		// 敵 Wizards の Petrificus と Flipendo
        if (!myTeamId) {
		...
		// ***************************************************************************************************
		// 遺伝的アルゴリズム開始
		// 01. 完全にランダムな開始母集団を生成する．
		// 02. すべての解をシミュレートし、フィットネス関数 (1409行目の "eval" 関数) を使用してスコアを付ける．
		// 03. ランダムに2つの別々のソリューションを選択する．
		// 04. 2つの中で最高のものをキープして、あなたの解答「A」を手に入れてください。
		// 05. 再びランダムに2つの別々の解答を選びます（解答Aとは異なることを確認してください）。
		// 06. この2つの中で一番良いものをキープしておきましょう。これが解答 "B" です．
		// 07. A と B から子ソリューションを作成します (156行目の "merge" 関数)
		// 08. 必要であれば、子ソリューションに突然変異を適用することができます。
		// 09. この子を次世代に配置する．
		// 10. ステップ03にジャンプバックして、完全に新世代になるまで繰り返す．
		// 11. 新世代のすべてのソリューションをシミュレートし、それらをスコア化する．
		// 12. 100ms が終了するまで生成を繰り返す．
        Solution* base;
        if (turn) {
            base = new Solution();
